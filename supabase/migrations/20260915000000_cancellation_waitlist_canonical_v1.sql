...
create index if not exists booking_waitlist_variant_idx
  on public.booking_waitlist (variant_id, status, created_at, id);
...
alter table public.notification_outbox
  add constraint notification_outbox_event_type_check check (
    event_type = any (array[
      'booking_requested','booking_confirmed','booking_cancelled','booking_updated',
      'attendance_recorded','price_updated','support_reply','waitlist_slot_opened','manual'
    ]::text[])
  ) not valid;
...
begin
  ...
  values (p_actor_id, p_patient_profile_id, v_offer.id, v_offer.variant_id)
...
begin
  ...
  return query
  select w.id, w.status, w.created_at, w.notified_at, tv.name_ar, tv.name_en
  from public.booking_waitlist w
...
begin
  ...
  where membership.clinic_id=v_booking.clinic_id and membership.status='active' and membership.role in ('owner','manager','receptionist')
  on conflict(dedupe_key) do nothing;

  if v_slot_released then
    for v_waitlist in
      select * from public.booking_waitlist
      where offer_id=v_booking.offer_id and status='active'
      order by created_at asc,id asc
    ...
    loop
      ...
      values(
        ...
        'in_app','ar',
        jsonb_build_object('booking_id',v_booking.id,'waitlist_id',v_waitlist.id,'slot_id',v_booking.slot_id,'offer_id',v_booking.offer_id,'clinic_id',v_booking.clinic_id,'start_at',v_booking.start_at,'booking_code',v_booking.booking_code),
...
-- The cancellation function is already server-only in the base schema; keep it server-only here too.
