-- Server-side operational commands. All functions validate the authenticated actor,
-- apply ownership checks, and are revoked from PUBLIC by default.
...
revoke all on function private.has_clinic_role(uuid, uuid, text[]) from public;

create or replace function public.request_offer_revision(
  p_offer_id uuid,
  p_price_type text,
  p_min_minor integer,
  p_max_minor integer,
...
as $$
declare
  current_offer public.branch_service_offers%rowtype;
  next_revision integer;
  revision_id uuid;
  proposed jsonb;
begin
  if auth.uid() is null then raise exception 'authentication required' using errcode = '28000'; end if;
  if p_price_type not in ('fixed','from','range','package','consultation_required') then raise exception 'invalid price type' using errcode = '22023'; end if;
  if p_duration_minutes < 5 or p_duration_minutes > 480 then raise exception 'invalid duration' using errcode = '22023'; end if;
  ...
  if not private.has_clinic_role((select b.clinic_id from public.branches b where b.id = current_offer.branch_id), current_offer.branch_id, array['owner','manager','pricing_manager']) then
  ...
  values (p_offer_id,next_revision,to_jsonb(current_offer),proposed,p_reason,'submitted',auth.uid())
  returning id into revision_id;
  return revision_id;
end;
$$;

create or replace function public.review_offer_revision(p_revision_id uuid, p_approve boolean, p_reason text default null)
returns uuid
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  values (p_booking_id,'checked_in',p_reason,auth.uid(),'clinic_ui','checkin')
  on conflict do nothing
  returning id into event_id;
  if event_id is null then
    select id into event_id from public.booking_attendance_events where booking_id=p_booking_id and event_type='checked_in' limit 1;
  end if;
  return event_id;
end;
$$;
...
begin
  ...
  insert into public.booking_attendance_events(booking_id,event_type,reason,recorded_by,source_type,source_id)
  values (p_booking_id,'attendance_reversed',p_reason,auth.uid(),case when private.is_platform_admin() then 'admin_ui' else 'clinic_ui' end,'reversal')
  returning id into event_id;
  return event_id;
end;
$$;

create or replace function public.create_settlement_period(p_clinic_id uuid, p_period_start date, p_period_end date, p_period_kind text, p_notes text default null)
returns uuid
language plpgsql
security definer
set search_path = ''
  ...
  select jsonb_build_object(
    ...
    'attended_bookings',(
      ...
      where b.clinic_id=p_clinic_id
        and exists (select 1 from public.booking_attendance_events ae where ae.booking_id=b.id and ae.event_type='checked_in')
        and not exists (select 1 from public.booking_attendance_events ar where ar.booking_id=b.id and ar.event_type='attendance_reversed')
        and b.start_at::date between p_start and p_end
    ),
    'posted_debit_minor',(
      select coalesce(sum(l.debit_minor),0) from public.accounting_journals j join public.accounting_journal_lines l on l.journal_id=j.id
      where j.clinic_id=p_clinic_id and j.status='posted' and j.occurred_at::date between p_start and p_end
    ),
    'posted_credit_minor',(
      select coalesce(sum(l.credit_minor),0) from public.accounting_journals j join public.accounting_journal_lines l on l.journal_id=j.id
      where j.clinic_id=p_clinic_id and j.status='posted' and j.occurred_at::date between p_start and p_end
    )
  ) into result;
  return result;
...
revoke all on function public.request_offer_revision(uuid,text,integer,integer,integer,text) from public;
revoke all on function public.review_offer_revision(uuid,boolean,text) from public;
revoke all on function public.record_booking_check_in(uuid,text) from public;
revoke all on function public.reverse_booking_attendance(uuid,text) from public;
revoke all on function public.create_settlement_period(uuid,date,date,text,text) from public;
revoke all on function public.financial_report_summary(uuid,date,date) from public;
grant execute on function public.request_offer_revision(uuid,text,integer,integer,integer,text) to authenticated;
grant execute on function public.review_offer_revision(uuid,boolean,text) to authenticated;
grant execute on function public.record_booking_check_in(uuid,text) to authenticated;
grant execute on function public.reverse_booking_attendance(uuid,text) to authenticated;
grant execute on function public.create_settlement_period(uuid,date,date,text,text) to authenticated;
