-- Do not expose a SECURITY DEFINER view. Keep the public RPC invoker-safe and
-- confine the private verification lookup to a fixed-search-path helper in the private schema.
drop view if exists public.public_searchable_clinics;

create or replace function private.is_public_searchable_clinic(p_clinic_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select not exists (
    ...
    where vr.subject_type = 'clinic'
      and vr.subject_id = p_clinic_id
      and vr.source = 'DEV_SYNTHETIC'
  );
$$;

revoke all on function private.is_public_searchable_clinic(uuid) from public;
grant execute on function private.is_public_searchable_clinic(uuid) to anon, authenticated;
...
as $$
  ...
  where o.variant_id = p_variant_id
    ...
    and b.status = 'active'
    and c.status = 'active'
    and private.is_public_searchable_clinic(c.id)
    and (
      p_lat is null
      or p_lng is null
