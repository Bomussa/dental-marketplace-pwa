alter table public.booking_attendance_events
  add column sequence_no integer not null;

alter table public.booking_attendance_events
  add constraint booking_attendance_events_sequence_no_check check (sequence_no > 0);

create unique index booking_attendance_booking_sequence_uidx
  on public.booking_attendance_events(booking_id, sequence_no);
  ...
  select jsonb_build_object(
    ...
    'attended_bookings',(
      ...
      where b.clinic_id=p_clinic_id
        and (select e.event_type from public.booking_attendance_events e where e.booking_id=b.id order by e.sequence_no desc limit 1)='checked_in'
