...
begin
  ...
  select * into v_profile from public.patient_profiles where id=p_patient_profile_id and account_id=v_user and archived_at is null for key share;
  if not found then raise exception 'patient profile is not available for this account' using errcode='42501'; end if;
  if v_profile.national_id is null or v_profile.nationality is null or v_profile.date_of_birth is null or v_profile.phone is null or v_profile.phone_verified_at is null then raise exception 'patient profile must be complete and phone verified' using errcode='22023'; end if;
  ...
  select * into v_entry from public.booking_waitlist where patient_profile_id=p_patient_profile_id and offer_id=p_offer_id and status='active' order by created_at asc limit 1;
  if found then
  ...
  insert into public.booking_waitlist(account_id,patient_profile_id,offer_id,variant_id) values(v_user,p_patient_profile_id,v_offer.id,v_offer.variant_id) returning * into v_entry;
  select count(*)+1 into v_position from public.booking_waitlist w where w.offer_id=v_entry.offer_id and w.status='active' and (w.created_at,w.id)<(v_entry.created_at,v_entry.id);
  insert into public.audit_events(actor_id,action,target_type,target_id,metadata) values(v_user,'booking.waitlist_joined','booking_waitlist',v_entry.id::text,jsonb_build_object('offer_id',v_entry.offer_id,'variant_id',v_entry.variant_id,'patient_profile_id',v_entry.patient_profile_id));
  return query select v_entry.id,v_entry.status,v_position;
