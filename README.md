...
| `/privacy` | عام | مسار الخصوصية الرسمي؛ يعرض إطاراً تقنياً واضحاً إلى أن يعتمد المالك/المختص النص القانوني النهائي. |
| `/terms` | عام | مسار شروط الاستخدام الرسمي؛ يعرض إطاراً تقنياً واضحاً إلى أن يعتمد المالك/المختص النص القانوني النهائي. |
...
| كتالوج العلاج | تحيط `getActiveTreatmentCatalog` قراءات الكتالوج المحكومة بمهلة بتعافٍ خادمي؛ عند مهلة عابرة يعيد صفيفين فارغين و`hasError=true` كي تعرض الصفحة تنبيه الخطأ المتاح بدلاً من رفض SSR بالكامل. شريط «العلاجات الشائعة» يبرز فقط التبييض وعلاج العصب والحشوة التجميلية وخلع السن وتنظيف الجير بترتيب ثابت؛ يستند أولاً إلى تجميع البحث المجهول الهوية المتاح وإلى إجراءات شائعة، مع بقاء جميع العلاجات النشطة في حقل الاختيار ولا يدعي ترتيباً عالمياً لعمليات البحث. |
...
4. يسجل المريض الدخول بـnickname وكلمة المرور، ثم ينشئ أو يحدّث ملف مريض. nickname المريض من 2 إلى 10 أحرف/أرقام إنجليزية وفريد، ولا يغير عقد أسماء دخول العيادة أو الإدارة.
...
7. عند تأكيد العيادة، تحفظ `notification_outbox` تأكيد `in_app` للمريض يضم العلاج والفرع والموقع والوقت ورابط الاتجاهات والتنبيه بالحضور قبل 30 دقيقة؛ هذه ليست رسالة SMS للحجز.

### 8.2 العيادة والإدارة

| السطح | ما يديره | الحد الفاصل |
|---|---|---|
| العيادة | طلب الانضمام، الفروع، ساعات العمل، الممارسون وجنسهم الموثق، المسودات، العروض، المواعيد وربطها بالممارس، طلب تعديل السعر، الحضور وحالة الحجز. | عضوية العيادة وRLS ومصفوفة انتقالات الحجز. |
...
| المدير الأعلى | كل ما يملكه مدير المنصة، وإنشاء حسابات المرضى والعملاء التشغيليين وإيقاف العملاء، وحذف حساب مريض مؤرشفًا مع أثر تدقيقي. | claimان: `platform_admin` و`platform_super_admin`، ثم إجراءات خادمية وRPC service-role فقط. |
| المريض | البحث والمقارنة العامة، ملفه وملفات التابعين والحجوزات والإشعارات المحفوظة والمراجعات، وحذف حسابه المؤكد. | ملكية الحساب، ملف غير مؤرشف، وهاتف موثق قبل الحجز؛ لا يحذف حساب مريض دور عيادة أو عميل تشغيلي. |
...
| الجلسات | Supabase Auth باسم المستخدم وكلمة المرور وcookies SSR؛ الواجهة العامة تستخدم جلسة العرض السريعة لتحديد رابط التنقل فقط، بينما يبقى `getServerAuthClaims` والتحقق الخادمي حكماً موثوقاً للتفويض. Proxy ينعش claims. يبقى `/auth/confirm` لتدفقات رمز الجلسة الخارجية فقط، وليس واجهة الدخول الاعتيادية. |
...
| تسجيل المريض | تتطلب البداية الاسم والرقم الشخصي والجنسية والهاتف وnickname فريدًا وبيانات الدخول؛ تستمد الجنسية من 249 رمز ISO 3166-1 alpha-2 في [`lib/account-nationality-options.ts`](lib/account-nationality-options.ts). nickname المريض 2–10 أحرف/أرقام إنجليزية، وكلمة المرور 4–10 أحرف أو أرقام إنجليزية؛ تسجيل الدخول يقبل 2–32 للحفاظ على توافق المرضى وحسابات التشغيل، بينما تبقى سياسات الإنشاء منفصلة. يمنع قيد قاعدة البيانات إعادة استخدام الرقم الشخصي أو الهاتف. تاريخ الميلاد والجنس اختياريان عند إنشاء الحساب، لكن تاريخ الميلاد يصبح لازمًا قبل تأكيد الحجز. |
| الحجز | RPC ذري ومفتاح idempotency وقيد ملف المريض/OTP. إشعار الحجز بعد تأكيد العيادة محفوظ داخل الحساب بقناة `in_app` ولا يستخدم SMS للحجز؛ OTP يبقى مسارًا أمنيًا منفصلًا. |
...
| إدارة المستخدمين | لا تُحفظ كلمة المرور أو تُعاد من الواجهة أو السجل. إنشاء الحسابات أعلى امتيازًا محمي بالـclaims. حذف حساب المريض مقصور على المريض نفسه أو المدير الأعلى، ويعطل الوصول ويؤرشف الملف ويحفظ الحد الأدنى من أثر الحجز والتدقيق بدل حذف تاريخ التشغيل. |
| CSP وPWA | اختبارات E2E تتحقق من CSP وعدم تحويل فشل API offline إلى HTML مخزّن. |

> **حالة هذه العقود:** نُشر كود ومخطط ميزات nickname وتفرد الهاتف والإشعارات الداخلية والخرائط وتفضيل جنس الممارس والحذف المقيد إلى Production في commit `7f814fc`، وسجّل Supabase الترحيلين تحت `20260826122625` و`20260826122705`. ظل الإثبات الوظيفي الكامل للحجز والإشعار والحذف محصورًا في Preview Staging ضمن `TEST_STG` ثم نُظفت fixture إلى صفر؛ لم يُنشأ أي حساب أو حجز أو OTP اختباري في Production. لا يزال OTP الحقيقي blocker تشغيليًا عند غياب تهيئة المزود. في جولة الإغلاق اللاحقة أضيفت بوابة جاهزية عامة قراءة فقط، اختبار axe WCAG A/AA، وسيناريو K6 محروس لإعادة المحاولة/السباق؛ كما طُبقت migration توحيد سياسة قراءة ملفات المرضى في Staging ثم Production بعد اعتماد صريح، بلا DML أو بيانات اختبار. لا يمثل ذلك تفعيل OTP أو تشغيل K6. راجع [`docs/CURRENT_PRODUCTION_STATUS.md`](docs/CURRENT_PRODUCTION_STATUS.md) للحكم الحالي وحدود الإثبات.
...
| `PRODUCTION_READINESS_SEARCH_VARIANT` | تحقق قراءة فقط | نوع علاجي منشور لاختبار شكل استجابة البحث في `production-readiness-check`; لا يكتب بيانات ولا يضع سراً. |
