...
| البحث والمقارنة | `app/page.tsx`، `app/results/page.tsx`، `app/api/search/route.ts`، `lib/search-offers.ts`، `lib/search-query.ts`، `lib/treatment-catalog.server.ts`، `components/search-form.tsx` | اختيار العلاج الدقيق، البحث، الفرز، النطاق المكاني والزمني، تفضيل جنس الممارس، رابط الاتجاهات من بيانات الفرع المنشورة، وcache الكتالوج. تبرز الواجهة خمسة علاجات شائعة في ترتيب ثابت، مع بقاء الكتالوج الكامل في حقل الاختيار. |
...
| API والحراس | `app/api/`، `lib/validation.ts`، `lib/public-write-request-guard.ts`، `lib/operations.server.ts`، `lib/server-readiness.ts` | Route Handlers وZod وrate limits والمهلات وعمليات الامتياز؛ `/api/health` يستخدم existence check محدوداً لتقليل العمل غير الضروري. |
...
| التحديث اللحظي | `components/*-live-refresh.tsx`، `components/use-realtime-router-refresh.ts`، `lib/clinic-realtime-refresh.ts` | إعادة الجلب المعتمدة على Realtime تحت RLS، مع coalescing وretry محدودين؛ زمن الحدث الفعلي يحتاج بيانات Staging حية لإثباته. |
...
| الاختبارات | `tests/`، `tests/featured-treatments.test.ts`، `tests/treatment-catalog-resilience.test.ts`، `tests/e2e/home.spec.ts`، `tests/e2e/accessibility.spec.ts`، `tests/production-readiness-check.test.ts`، `load-tests/k6/`، `scripts/safe-load-test.mjs` | Vitest وPlaywright وaxe WCAG A/AA وK6 المحروس؛ اختبار العلاجات البارزة يثبت ترتيبها وبديل الكتالوج، واختبار الكتالوج يثبت تحويل مهلة عابرة إلى حالة خطأ واجهة بدلاً من رفض SSR. لا يستخدم K6 على Production. |
