...
بدأ اختبار حي جديد لمسار تأكيد الدخول بعد إصدار `da97b32` عبر صندوق بريد اصطناعي مستقل، مع إرسال رابط واحد فقط إلى الحساب المؤقت وعدم إنشاء أي حجز أو تحقق هاتف.
أثبت طلب HTTP حي إلى `/?code=live-routing-probe-invalid` استجابة `307` مع الوجهة `/auth/confirm?code=live-routing-probe-invalid`، وهو ما يثبت تحويل الحاجز المنشور دون الحاجة إلى رمز صالح. أما محاولة إرسال رابط بريد جديد بعد الإصلاح فأعادت `send_failed` من مزود المصادقة قبل إنشاء أي مستخدم في `auth.users`. لم تسجل بيئة التطبيق أخطاء وقت تشغيل، وتعذر استخراج سجل تفصيلي من موفر المصادقة بسبب خطأ في واجهة السجل نفسها. لذلك لا يُدّعى نجاح تسليم بريد هذه المحاولة؛ يظل التحقق الحي السابق مثبتًا لوصول رابط وإنشاء جلسة، أما اختبار التسليم الجديد فمحجوب حاليًا خارج التطبيق ويحتاج استعادة خدمة البريد لدى موفر المصادقة أو الانتظار لانتهاء حد الإرسال.
أعاد فحص الصحة الأخير على الدومين الرسمي `ok=true` و`database=true` و`server_operations=true` مع وجود 25 علاجًا في الكتالوج، بعد تنظيف كل بيانات الاختبار الاصطناعية.
