...
## 8. تحقق مباشر من الموقع المنشور

تم فتح `https://dental-marketplace-pwa.vercel.app` في 15 أغسطس 2026 والتحقق من تحميل الصفحة العربية العامة ومن انتقال زر **عرض النتائج** إلى مسار النتائج. أعاد البحث الافتراضي **0 نتيجة مؤهلة** ورسالة صريحة تفيد بأن ذلك لا يعني عدم وجود الخدمة في قطر؛ وهو السلوك الآمن المتوقع عند عدم وجود عرض حي مناسب، ولا يمثل بيانات وهمية. هذا الموقع المنشور يعكس الإصدار المتاح حاليًا؛ الدفعة الجديدة موجودة في طلب الدمج رقم 6 ولم تُنشر إلى Vercel بعد.

## 9. تحسين الأداء اللاحق

بعد فحص مستشار الأداء، أضيفت الهجرة `20260815075000_operational_performance_indexes_rls_v1.sql`. تغطي الهجرة فهارس مفاتيح التشغيل الأجنبية وتعيد صياغة سياسات RLS التشغيلية لتستخدم `(select auth.uid())` و`(select auth.jwt())`، مع إبقاء نطاق الوصول نفسه. أزال الفحص اللاحق تحذيرات المفاتيح الأجنبية غير المفهرسة وتحذيرات إعادة تقييم المصادقة؛ الملاحظات المتبقية هي **معلومات** عن فهارس لم تسجل استخدامًا بعد في بيئة DEV ذات الحمل المحدود، وليست ثغرات أو أخطاء تشغيلية. أعاد مستشار الأمان النتيجة `lints: []` بعد هذه الهجرة.
