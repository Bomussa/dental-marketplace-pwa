...
*الصورة 2 — لقطة سابقة لشاشة الدخول. الواجهة الحالية تطلب اسم المستخدم وكلمة المرور؛ لا تُخزن كلمة المرور في ملف المريض أو العيادة.*
...
5. اضغط **«قارن الخيارات الآن»**.

![لقطة حية: البحث الإنجليزي](assets/01-home-en-live.webp)

*الصورة 3 — الواجهة الحية نفسها بالإنجليزية. يطابق ترتيب الحقول النسخة العربية مع انعكاس اتجاه الواجهة.*

### 3.2 كيف تقرأ النتيجة

تظهر بطاقة العرض المؤهل فقط عندما يكون النوع العلاجي مطابقًا وعرض السعر صالحًا وموعده قابلًا للحجز. قبل طلب الحجز، راجع الاسم، الفرع، مدة الخدمة، المسافة عند استخدام الموقع، وقت آخر تحقق، ونطاق السعر بالتفصيل.
...
*الصورة 4 — حالة نتائج بلا عروض مؤهلة. استخدم زر «بحث جديد» بدل تكرار الطلب نفسه.*

## 4. المريض: البحث وطلب الحجز والحساب

### 4.1 طلب موعد

بعد ظهور عرض يملك موعدًا صالحًا، اضغط **«احجز هذا الموعد»**. إذا لم تكن مسجّلًا، تظهر خطوة تسجيل داخل بطاقة الحجز نفسها قبل أي تأكيد للموعد. أدخل البريد الإلكتروني واسم المستخدم وكلمة المرور، ثم أكمل بيانات المريض. لا ينشأ الحجز في هذه المرحلة؛ يُنشأ الحساب والملف الذاتي فقط، ثم يظل تأكيد الهاتف والموعد مطلوبين.

أكمل بيانات المريض كما هي في المستندات الرسمية:

| الحقل | المطلوب |
|---|---|
| الاسم | الاسم الظاهر للمريض. |
| العلاقة | `self` لنفس صاحب الحساب، أو العلاقة المناسبة للتابع. |
| الرقم الشخصي | رقم صحيح وفريد للملف النشط؛ لا تُنشئ ملفًا مكررًا. |
| الجنسية | اخترها من القائمة. |
| تاريخ الميلاد | تاريخ المريض الحقيقي. |
| الهاتف | رقم يمكن التحقق منه عبر OTP. |
| الجنس | اختياري عندما يظهر في النموذج. |

بعد الحفظ اضغط **«حفظ البيانات وإرسال رمز التحقق»**، ثم أدخل الرمز واضغط **«تأكيد الحجز»**. لا يُنشأ الحجز ما لم يكن الملف كاملًا و`phone_verified_at` مثبتًا.
...
*الصورة 5 — المثال الحقيقي الذي يوضح أن لوحة العيادة لا تفتح للزائر. بعد الدخول بدور مخوّل فقط تظهر لوحة التشغيل.*

### 5.1.1 حسابا التشغيل للعيادة
...
يحصل كل حساب تشغيلي على دور **مدير عيادة** داخل المركز نفسه، فيستطيع تنفيذ إجراءات التشغيل المسموحة للمدير في الفروع المخوّلة، لكنه لا يملك أي صلاحية للإدارة العامة للمنصة ولا يمكنه إنشاء حساب تشغيلي ثالث. يعرض القسم اسم المستخدم وحالة الحساب، ويمكن للمالك اختيار **«إلغاء الحساب»**؛ عندها تُسحب العضوية التشغيلية فورًا وتبقى عملية الإلغاء مسجلة لأغراض التدقيق.
...
3. في بطاقة **«إضافة ممارس»** اكتب الاسم ومرجع الترخيص إن توفر، ثم احفظ. يظهر الممارس بحالة معلّقة حتى التحقق الإداري.
...
1. في بطاقة **«موعد جديد»** اختر الفرع والنوع العلاجي.
...
*الصورة 6 — لا يحصل الزائر على شاشة الإدارة. اللقطة تثبت حاجز الوصول، وليست ادعاءً بامتلاك صلاحية مدير.*
...
**قوالب الإشعارات:** أنشئ مسودة من مفتاح قالب وقناة (`push` أو`email` أو`sms`) ولغة وموضوع اختياري ونص الرسالة، ثم احفظ واضغط **«تفعيل القالب»**. تفعيل قالب لا يثبت تسليم رسالة خارجية؛ راجع **صندوق الإرسال** وحالة المحاولة. لا توجد أداة تعطيل قالب مباشر ضمن البطاقة الحالية؛ لا تعد العميل بوجودها.
...
| لا توجد عروض | لا توجد عروض عامة مؤهلة للشروط الحالية. | غيّر الوقت أو الموقع أو النوع؛ لا تضف سعرًا وهميًا. |
...
| المصدر | ماذا يثبت |
|---|---|
| [`app/clinic/page.tsx`](../../app/clinic/page.tsx) و[`app/clinic/actions.ts`](../../app/clinic/actions.ts) | أقسام لوحة العيادة، الأدوار، ونماذج الفروع والعروض والمواعيد والحجوزات وحسابي التشغيل. |
...
**سياسة التحديث:** عند إضافة زر أو حالة أو دور أو تكامل جديد، يحدّث هذا الكتيب في الإيداع نفسه مع لقطة حقيقية مخوّلة أو مع توضيح صريح بأن العنصر مقيد ولا توجد لقطة تشغيلية له.

## 10. ملحق التغطية الكاملة للإجراءات

هذا الملحق يربط كل Server Action ظاهر في أسطح التطبيق بالدور والإجراء المشروح أعلاه. لا يعني وجود الدالة أن كل مستخدم يستطيع تنفيذها؛ الصلاحية الفعلية تبقى محكومة بالجلسة وRLS وحالة السجل.

| الإجراء الفعلي | من يراه/يطلبه | الإجراء المبسّط في الكتيب | المرجع |
|---|---|---|---|
...
| `createClinicOperatorAccount` | مالك عيادة نشطة | أنشئ أحد حسابي التشغيل فقط. | [5.1.1](#511-حسابا-التشغيل-للعيادة) |
| `revokeClinicOperatorAccount` | مالك العيادة | اسحب حساب تشغيل قائم. | [5.1.1](#511-حسابا-التشغيل-للعيادة) |
