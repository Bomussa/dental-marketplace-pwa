...
| بحث وتسعير | `search-query.ts`، `search-offers.ts`، `treatment-catalog.server.ts`، `price.ts`، `price-scope.ts`، `money-input.ts` | تحليل query، cache الكتالوج وتعافي مهلة القراءة إلى حالة خطأ واجهة، ترتيب العلاجات الشائعة في واجهة البحث مع بقاء الكتالوج كاملاً، تحويل المال، ونطاق السعر. |
...
| `tests/*.test.ts` و`tests/featured-treatments.test.ts` و`tests/treatment-catalog-resilience.test.ts` و`tests/e2e/home.spec.ts` و`tests/mocks/server-only.ts` | اختبارات وحدات/عقود/E2E، بما فيها ترتيب العلاجات الشائعة وانحدار مهلة كتالوج العلاج، وحاجز modules الخادمية. |
