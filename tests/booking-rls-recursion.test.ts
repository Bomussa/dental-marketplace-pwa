...
const searchPathMigrationSql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/20260826194500_booking_patient_rls_helper_search_path_v1.sql"),
  "utf8",
);
...
describe("booking RLS recursion regression", () => {
  it("keeps the archived self-patient guard in a private definer helper", () => {
    ...
    expect(searchPathMigrationSql).toContain("set search_path to pg_catalog");
    expect(searchPathMigrationSql).toContain("grant execute on function private.has_active_self_patient_profile(uuid) to authenticated;");
  });
