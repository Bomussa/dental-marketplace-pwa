...
export async function POST(request: Request) {
  ...
  if (publicWriteRequestBodyIsTooLarge(request, MAX_DEVICE_INSTALLATION_BYTES)) {
    return jsonNoStore({ error: "device_installation_too_large" }, 413);
  }

  const raw = await readPublicWriteRequestTextWithinLimit(request, MAX_DEVICE_INSTALLATION_BYTES).catch(() => "");
  if (raw === null) return jsonNoStore({ error: "device_installation_too_large" }, 413);

  let body: unknown = null;
  try {
    body = raw ? JSON.parse(raw) : null;
  } catch {
    return jsonNoStore({ error: "بيانات التثبيت غير صالحة" }, 400);
  }

  const parsed = deviceInstallationSchema.safeParse(body);
  if (!parsed.success) return jsonNoStore({ error: "بيانات التثبيت غير صالحة" }, 400);
  ...
  try {
    ...
    return jsonNoStore({ error: "تعذر تحديث التثبيت" }, 503);
  }
