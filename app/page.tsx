    ...
    <main className="overflow-hidden">
      <section className="relative px-4 pb-14 pt-16 sm:px-6 sm:pb-20 sm:pt-24">
        ...
        <div className="relative mx-auto max-w-7xl">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mb-3 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-[.18em] text-[#084884]"><SparklesIcon size={16}/> {t["home.intelligence"]}</p>
            <h1 className="mx-auto max-w-4xl text-[2.65rem] font-black leading-[1.08] tracking-[-.035em] text-slate-950 sm:text-6xl lg:text-7xl">
              {t["home.hero.before"]} <span className="text-[#0B5CAD]">{t["home.hero.highlight"]}</span> {t["home.hero.after"]}
            </h1>
          ...
          <div className="mx-auto mt-5 flex max-w-4xl flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs font-bold text-slate-500">
            <span className="inline-flex items-center gap-1.5"><ShieldCheckIcon size={16} className="text-[#0B5CAD]"/> {t["home.trust.verified"]}</span>
            <span className="inline-flex items-center gap-1.5"><WalletIcon size={16} className="text-[#0B5CAD]"/> {t["home.trust.payment"]}</span>
            <span className="inline-flex items-center gap-1.5"><CalendarIcon size={16} className="text-[#0B5CAD]"/> {t["home.trust.booking"]}</span>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-slate-200/70 bg-white/55 px-4 py-16 sm:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-2xl"><p className="text-xs font-black uppercase tracking-[.18em] text-[#084884]">{t["home.decision.kicker"]}</p><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">{t["home.decision.title"]}</h2></div>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ...
              return <Card key={String(title)} className="lift p-6"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-50 text-[#0B5CAD]"><FeatureIcon size={21}/></span><h3 className="mt-5 text-lg font-black">{String(title)}</h3><p className="mt-2 text-sm font-medium leading-7 text-slate-500">{String(copy)}</p></Card>;
            })}
          </div>
        </div>
      </section>
