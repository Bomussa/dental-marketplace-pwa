...
export default async function AccountPage({ searchParams }: { searchParams: Promise<AccountSearchParams> }) {
  ...
  const patientProfileErrorMessage = params.patient_profile_error === "self_exists"
    ...
    : params.patient_profile_error === "duplicate_identity"
      ? "الرقم الشخصي مسجل مسبقًا في ملف آخر."
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-9 sm:px-6 sm:py-12">
      <section className="glass-panel rounded-[30px] p-5 sm:p-7">
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="flex min-w-0 items-center gap-4">
            <span className="grid h-14 w-14 shrink-0 place-items-center rounded-[20px] bg-blue-50 text-[#0B5CAD]"><UserIcon size={26}/></span>
            <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[.16em] text-[#084884]">حساب المريض</p><h1 className="mt-1 truncate text-3xl font-black tracking-tight">{profile?.display_name || "حسابي"}</h1><p className="mt-1 text-sm font-medium text-slate-500">ملف شخصي مُقلّل البيانات، بدون تشخيصات أو صور أشعة أو وصفات.</p></div>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3"><AccountLiveRefresh userId={userId}/><form action="/auth/signout" method="post"><button className="min-h-11 rounded-full bg-white px-4 text-sm font-extrabold text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-slate-50">تسجيل الخروج</button></form></div>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl bg-blue-50/75 p-4 ring-1 ring-blue-100"><div className="text-2xl font-black text-[#084884]">{upcoming}</div><div className="mt-1 text-xs font-bold text-slate-500">قادمة أو قيد التأكيد</div></div>
      ...
      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-[#084884]">Family profiles</p><h2 className="mt-1 text-2xl font-black">لمن تحجز المواعيد؟</h2><p className="mt-1 text-sm font-medium text-slate-500">أضف بيانات الحجز الأساسية لكل شخص. لا نخزن تشخيصات أو صور أشعة أو وصفات هنا.</p></div><UserIcon className="text-[#0B5CAD]" size={24}/></div>
        ...
        <Card className="p-5 sm:p-6">
          <div className="grid gap-3 sm:grid-cols-2">{patientProfiles.map((patientProfile) => <div key={patientProfile.id} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200/70"><div><p className="font-black">{patientProfile.display_name}</p><p className="mt-1 text-xs font-bold text-slate-500">{patientProfile.relationship === "self" ? "أنا" : patientProfile.relationship === "child" ? "ابن/ابنة" : patientProfile.relationship === "spouse" ? "زوج/زوجة" : patientProfile.relationship === "parent" ? "أب/أم" : "فرد من العائلة"} · {patientProfile.nationality ?? "—"}</p><p className="mt-1 text-xs font-bold text-slate-500" dir="ltr">{patientProfile.phone ?? "—"}</p></div><div className="flex items-center gap-2"><Badge tone={patientProfile.phone_verified_at ? "green" : "amber"}>{patientProfile.phone_verified_at ? "هاتف متحقق" : "يتطلب تحققًا عند الحجز"}</Badge>{patientProfile.relationship !== "self" && <form action={archivePatientProfile}><input type="hidden" name="patient_profile_id" value={patientProfile.id}/><button className="min-h-11 rounded-full px-3 text-xs font-black text-red-700 ring-1 ring-red-200 transition hover:bg-red-50">أرشفة</button></form>}</div></div>)}</div>
          <form action={createPatientProfile} className="mt-5 grid gap-3 border-t border-slate-200/70 pt-5 sm:grid-cols-2">
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">الاسم الكامل<Input name="display_name" required maxLength={120} placeholder="اسم الشخص كما سيظهر"/></label>
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">صلة القرابة<Select name="relationship" defaultValue="child"><option value="child">ابن/ابنة</option><option value="spouse">زوج/زوجة</option><option value="parent">أب/أم</option><option value="other">فرد آخر</option></Select></label>
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">الرقم الشخصي<Input name="national_id" inputMode="numeric" required pattern="[0-9٠-٩۰-۹ -]{11,20}" placeholder="11 رقمًا"/></label>
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">الجنسية<Select name="nationality" required defaultValue=""><option value="">اختر الجنسية</option><option value="QA">قطر</option><option value="SA">السعودية</option><option value="AE">الإمارات</option><option value="BH">البحرين</option><option value="KW">الكويت</option><option value="OM">عُمان</option><option value="EG">مصر</option><option value="IN">الهند</option><option value="PH">الفلبين</option><option value="PK">باكستان</option><option value="BD">بنغلاديش</option><option value="JO">الأردن</option><option value="LB">لبنان</option><option value="SY">سوريا</option><option value="US">الولايات المتحدة</option><option value="GB">المملكة المتحدة</option></Select></label>
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">تاريخ الميلاد<Input name="date_of_birth" type="date" required/></label>
            <label className="grid gap-1 text-xs font-extrabold text-slate-600">رقم الهاتف<Input name="phone" type="tel" dir="ltr" required placeholder="+974XXXXXXXX"/></label>
            <div className="sm:col-span-2"><Button>حفظ ملف المريض</Button><p className="mt-2 text-xs font-bold text-slate-500">يُطلب رمز تحقق فعلي لرقم الهاتف عند تأكيد أول حجز لهذا الملف.</p></div>
          </form>
        </Card>
      </section>

      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-[#084884]">Bookings</p><h2 className="mt-1 text-2xl font-black">حجوزاتي</h2></div><CalendarIcon className="text-[#0B5CAD]" size={24}/></div>
