...
export default async function AccountPage({ searchParams }: { searchParams: Promise<AccountSearchParams> }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-9 sm:px-6 sm:py-12">
      <section className="glass-panel rounded-[30px] p-5 sm:p-7">
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[.16em] text-[#084884]">{copy.accountKicker}</p><h1 className="mt-1 truncate text-3xl font-black tracking-tight">{profile?.display_name || copy.defaultName}</h1><p className="mt-1 text-sm font-medium text-slate-500">{copy.privacy}</p></div>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3"><AccountLiveRefresh userId={userId} /><form action="/auth/signout" method="post"><button className="min-h-11 rounded-full bg-white px-4 text-sm font-extrabold text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-slate-50">{copy.signOut}</button></form></div>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-white/75 p-4 ring-1 ring-slate-200/60"><div className="text-2xl font-black">{bookings.length}</div><div className="mt-1 text-xs font-bold text-slate-500">{copy.allBookings}</div></div>
          <div className="rounded-2xl bg-blue-50/75 p-4 ring-1 ring-blue-100"><div className="text-2xl font-black text-[#084884]">{upcoming}</div><div className="mt-1 text-xs font-bold text-slate-500">{copy.upcoming}</div></div>
          <div className="rounded-2xl bg-emerald-50/75 p-4 ring-1 ring-emerald-100"><div className="text-2xl font-black text-emerald-700">{completed}</div><div className="mt-1 text-xs font-bold text-slate-500">{copy.completedVisits}</div></div>
        </div>
      </section>

      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-[#084884]">{copy.familyKicker}</p><h2 className="mt-1 text-2xl font-black">{copy.familyTitle}</h2><p className="mt-1 text-sm font-medium text-slate-500">{copy.familyCopy}</p></div><UserIcon className="text-[#0B5CAD]" size={24} /></div>
      ...
      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-[#084884]">{copy.bookingsKicker}</p><h2 className="mt-1 text-2xl font-black">{copy.bookingsTitle}</h2></div><CalendarIcon className="text-[#0B5CAD]" size={24} /></div>
