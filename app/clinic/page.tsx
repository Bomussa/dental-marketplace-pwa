  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12">
      ...
      <section className="mt-7 grid gap-6 lg:grid-cols-3">
        <Card className="p-5"><div className="flex items-center gap-2"><BuildingIcon size={18} className="text-[#007AFF]"/><h2 className="font-black">إضافة فرع</h2></div><form action={createBranch} className="mt-4 grid gap-3"><input type="hidden" name="clinic_id" value={selectedClinic?.id}/><Input name="name" required placeholder="اسم الفرع"/><Input name="area" placeholder="المنطقة"/><Input name="address_line" placeholder="العنوان"/><div className="grid grid-cols-2 gap-2"><Input name="lat" type="number" step="any" placeholder="Latitude" dir="ltr"/><Input name="lng" type="number" step="any" placeholder="Longitude" dir="ltr"/></div><Button>حفظ الفرع</Button></form></Card>
        <Card className="p-5"><div className="flex items-center gap-2"><UserIcon size={18} className="text-[#007AFF]"/><h2 className="font-black">إضافة طبيب</h2></div><form action={createPractitioner} className="mt-4 grid gap-3"><input type="hidden" name="clinic_id" value={selectedClinic?.id}/><Input name="display_name" required placeholder="اسم مقدم الخدمة"/><Input name="license_ref" placeholder="مرجع الترخيص"/><Button>حفظ Pending</Button></form><div className="mt-4 space-y-2 text-xs">{(practitionerData??[]).map((p:{id:string;display_name:string;active:boolean})=><div key={p.id} className="flex justify-between rounded-2xl bg-slate-50/80 p-3 ring-1 ring-slate-200/60"><span className="font-bold">{p.display_name}</span><Badge tone={p.active?"green":"amber"}>{p.active?"active":"pending"}</Badge></div>)}</div></Card>
