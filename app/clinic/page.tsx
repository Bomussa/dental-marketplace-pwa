...
export default async function ClinicPage({ searchParams }: { searchParams: Promise<{ clinic?: string }> }) {
  ...
  const offers=(offerData??[]) as Offer[], slots=(slotData??[]) as Slot[], variants=(variantData??[]) as Variant[], bookings=(bookingData??[]) as Booking[], notifications=(notificationData??[]) as ClinicNotification[];
  const rolesForBranch = (branchId: string) => memberships
    .filter((membership) => membership.clinic_id === selectedClinicId && (membership.branch_id === null || membership.branch_id === branchId))
  ...
  if (bookingIds.length) {
    const { data: patientDetailsData } = await supabase.rpc("clinic_booking_patient_details", { p_booking_ids: bookingIds });
    for (const patientDetails of (patientDetailsData ?? []) as BookingPatientDetails[]) {
      if (patientDetails.patient_display_name) patientDetailsByBooking.set(patientDetails.booking_id, patientDetails);
    }
  }
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12">
      <section className="glass-panel rounded-[30px] p-5 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-5"><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-blue-50 text-[#0B5CAD]"><BuildingIcon size={24}/></span><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#084884]">لوحة العيادة</p><h1 className="mt-1 text-3xl font-black tracking-tight">{selectedClinic?.display_name??"عيادتي"}</h1><div className="mt-3 flex flex-wrap gap-2"><Badge tone={selectedClinic?.status==="active"?"green":"amber"}>{selectedClinic?.status??"pending"}</Badge><Badge>{memberships.find((membership) => membership.clinic_id === selectedClinicId)?.role ?? "member"}</Badge></div></div></div><div className="flex max-w-md flex-col items-start gap-3"><ClinicLiveRefresh branchIds={branchIds} userId={userId}/><p className="text-xs font-medium leading-6 text-slate-500">إدارة الفروع والأطباء والأسعار والمواعيد والحجوزات من مساحة تشغيل واحدة. لا تظهر أو تعمل أدوات التعديل إلا ضمن دورك وفرعك المخول.</p></div></div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-4"><div className="rounded-2xl bg-white/75 p-4 ring-1 ring-slate-200/60"><div className="text-2xl font-black">{branches.length}</div><div className="mt-1 text-xs font-bold text-slate-500">فروع</div></div><div className="rounded-2xl bg-blue-50/75 p-4 ring-1 ring-blue-100"><div className="text-2xl font-black text-[#084884]">{publishedOffers}</div><div className="mt-1 text-xs font-bold text-slate-500">عروض منشورة</div></div><div className="rounded-2xl bg-emerald-50/75 p-4 ring-1 ring-emerald-100"><div className="text-2xl font-black text-emerald-700">{publishedSlots}</div><div className="mt-1 text-xs font-bold text-slate-500">مواعيد منشورة</div></div><div className="rounded-2xl bg-white/75 p-4 ring-1 ring-slate-200/60"><div className="text-2xl font-black">{bookings.length}</div><div className="mt-1 text-xs font-bold text-slate-500">حجوزات ظاهرة لك</div></div></div>
      </section>

      <section className="mt-7 grid gap-6 lg:grid-cols-3">
        <Card className="p-5"><div className="flex items-center gap-2"><BuildingIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">إضافة فرع</h2></div><form action={createBranch} className="mt-4 grid gap-3"><input type="hidden" name="clinic_id" value={selectedClinic?.id}/><Input name="name" required placeholder="اسم الفرع"/><Input name="area" placeholder="المنطقة"/><Input name="address_line" placeholder="العنوان"/><div className="grid grid-cols-2 gap-2"><Input name="lat" type="number" step="any" placeholder="Latitude" dir="ltr"/><Input name="lng" type="number" step="any" placeholder="Longitude" dir="ltr"/></div><Button disabled={!canManageClinic} title={!canManageClinic ? "يتطلب هذا الإجراء دور المالك أو المدير وعيادة مفعلة" : undefined}>حفظ الفرع</Button></form></Card>
        <Card className="p-5"><div className="flex items-center gap-2"><UserIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">إضافة طبيب</h2></div><form action={createPractitioner} className="mt-4 grid gap-3"><input type="hidden" name="clinic_id" value={selectedClinic?.id}/><Input name="display_name" required placeholder="اسم مقدم الخدمة"/><Input name="license_ref" placeholder="مرجع الترخيص"/><Button disabled={!canManageClinic} title={!canManageClinic ? "يتطلب هذا الإجراء دور المالك أو المدير وعيادة مفعلة" : undefined}>حفظ Pending</Button></form><div className="mt-4 space-y-2 text-xs">{(practitionerData??[]).map((p:{id:string;display_name:string;active:boolean})=><div key={p.id} className="flex justify-between rounded-2xl bg-slate-50/80 p-3 ring-1 ring-slate-200/60"><span className="font-bold">{p.display_name}</span><Badge tone={p.active?"green":"amber"}>{p.active?"active":"pending"}</Badge></div>)}</div></Card>
        <Card className="p-5"><div className="flex items-center gap-2"><ClockIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">الفروع وساعات العمل</h2></div><div className="mt-4 grid gap-3">{branches.length?branches.map(b=><div key={b.id} className="rounded-[20px] bg-slate-50/80 p-4 ring-1 ring-slate-200/60"><div className="flex items-start justify-between gap-2"><div><div className="font-black">{b.name}</div><div className="mt-1 text-xs font-bold text-slate-500">{b.area||"—"}</div></div><Badge tone={b.status==="active"?"green":"amber"}>{b.status}</Badge></div>{b.status==="active" && canManageSlots(b.id) && <form action={setDailyHours} className="mt-3 grid grid-cols-[1fr_1fr_auto] gap-2"><input type="hidden" name="branch_id" value={b.id}/><Input name="open_time" type="time" defaultValue="08:00" required/><Input name="close_time" type="time" defaultValue="20:00" required/><Button className="px-4">حفظ</Button></form>}</div>):<p className="text-sm text-slate-500">لا توجد فروع بعد.</p>}</div></Card>
      </section>

      {branches.length ? <section className="mt-6 grid gap-6 lg:grid-cols-2"><Card className="p-5"><div className="flex items-center gap-2"><WalletIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">عرض سعر جديد</h2></div><form action={createOffer} className="mt-4 grid gap-3"><Select name="branch_id" required disabled={!offerWritableBranches.length}>{offerWritableBranches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</Select><Select name="variant_id" required>{variants.map(v=><option key={v.id} value={v.id}>{v.name_ar}</option>)}</Select><Select name="price_type" required><option value="fixed">سعر ثابت</option><option value="from">يبدأ من</option><option value="range">نطاق</option><option value="package">باقة</option><option value="consultation_required">بعد الاستشارة</option></Select><div className="grid grid-cols-2 gap-2"><Input name="min_qar" type="number" step="0.01" min="0" placeholder="الأدنى QAR"/><Input name="max_qar" type="number" step="0.01" min="0" placeholder="الأعلى QAR"/></div><Input name="duration_minutes" type="number" min="5" max="480" defaultValue="30" required/><Button disabled={!offerWritableBranches.length}>حفظ كمسودة</Button></form></Card><Card className="p-5"><div className="flex items-center gap-2"><CalendarIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">موعد جديد</h2></div><form action={createSlot} className="mt-4 grid gap-3"><Select name="branch_id" required disabled={!slotWritableBranches.length}>{slotWritableBranches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</Select><Select name="variant_id" required>{variants.map(v=><option key={v.id} value={v.id}>{v.name_ar}</option>)}</Select><label className="grid gap-1 text-xs font-extrabold">البداية<Input name="start_at" type="datetime-local" required/></label><label className="grid gap-1 text-xs font-extrabold">النهاية<Input name="end_at" type="datetime-local" required/></label><Button disabled={!slotWritableBranches.length}>حفظ كمسودة</Button></form></Card></section> : null}

      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card className="p-5">
          <div className="flex items-center gap-2"><WalletIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">عروض الأسعار</h2></div>
          <div className="mt-4 space-y-3 text-sm">
            {offers.length ? offers.slice(0, 12).map((offer) => (
              <div key={offer.id} className="rounded-[18px] bg-slate-50/80 p-3 ring-1 ring-slate-200/60">
                <div className="flex items-center justify-between gap-3"><span className="font-bold">{offer.price_type} · {offer.duration_minutes} د · {offer.min_minor === null ? "بعد الاستشارة" : `${(offer.min_minor / 100).toFixed(2)} ر.ق`}</span><div className="flex items-center gap-2"><Badge tone={offer.status === "active" ? "green" : "slate"}>{offer.status}</Badge>{offer.status === "draft" && canPublishOffers(offer.branch_id) && <form action={publishOffer}><input type="hidden" name="id" value={offer.id}/><Button className="min-h-9 px-3 py-1 text-xs">نشر</Button></form>}</div></div>
                {offer.status === "active" && canManageOffers(offer.branch_id) && <form action={requestPriceRevision} className="mt-3 grid gap-2 border-t border-slate-200 pt-3 sm:grid-cols-5"><input type="hidden" name="offer_id" value={offer.id}/><input type="hidden" name="price_type" value={offer.price_type}/><input type="hidden" name="duration_minutes" value={offer.duration_minutes}/><Input name="min_qar" type="number" min="0" step="0.01" defaultValue={(offer.min_minor ?? 0) / 100} aria-label="السعر الأدنى الجديد"/><Input name="max_qar" type="number" min="0" step="0.01" defaultValue={(offer.max_minor ?? offer.min_minor ?? 0) / 100} aria-label="السعر الأعلى الجديد"/><Input name="reason" className="sm:col-span-2" minLength={3} required placeholder="سبب التعديل"/><Button className="px-3 text-xs">طلب تعديل</Button></form>}
              </div>
            )) : <span className="text-slate-500">لا توجد عروض.</span>}
          </div>
        </Card>
        <Card className="p-5"><div className="flex items-center gap-2"><CalendarIcon size={18} className="text-[#0B5CAD]"/><h2 className="font-black">المواعيد المنشورة والمسودات</h2></div><div className="mt-4 space-y-2 text-sm">{slots.length?slots.slice(0,12).map(s=><div key={s.id} className="flex items-center justify-between gap-3 rounded-[18px] bg-slate-50/80 p-3 ring-1 ring-slate-200/60"><span className="font-bold">{new Intl.DateTimeFormat("ar-QA",{dateStyle:"short",timeStyle:"short",timeZone:"Asia/Qatar"}).format(new Date(s.start_at))}</span><div className="flex items-center gap-2"><Badge tone={s.status==="published"?"green":"slate"}>{s.status}</Badge>{s.status==="draft" && canManageSlots(s.branch_id) && <form action={publishSlot}><input type="hidden" name="id" value={s.id}/><Button className="min-h-9 px-3 py-1 text-xs">نشر</Button></form>}</div></div>):<span className="text-slate-500">لا توجد مواعيد.</span>}</div></Card>
      </section>

      <Card className="mt-6 p-5 sm:p-6">
        <div className="flex items-center gap-2"><SlidersIcon size={19} className="text-[#0B5CAD]"/><h2 className="font-black">الحجوزات التشغيلية</h2></div>
        ...
        {notifications.length > 0 && <div className="mt-4 grid gap-2">{notifications.map((notification) => { const payload = (notification.payload ?? {}) as Record<string, unknown>; return <div key={notification.id} className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3 text-xs font-bold text-blue-900"><span>حجز جديد ينتظر التأكيد: <b>{String(payload.treatment_name ?? "العلاج المختار")}</b></span><span dir="ltr">{new Intl.DateTimeFormat("ar-QA", { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Qatar" }).format(new Date(notification.created_at))}</span></div>; })}</div>}
        <div className="mt-4 space-y-3">{bookings.length ? bookings.map((b) => {
          ...
          const patientDetails = patientDetailsByBooking.get(b.id);
          const canOperate = canOperateBooking(b.branch_id);
          return <div key={b.id} className="grid gap-3 rounded-[20px] bg-slate-50/80 p-4 ring-1 ring-slate-200/60 sm:grid-cols-[1fr_auto]">
            <div><div className="font-black" dir="ltr">{b.booking_code}</div><div className="mt-1 text-xs font-bold text-slate-500">{new Intl.DateTimeFormat("ar-QA",{dateStyle:"short",timeStyle:"short",timeZone:"Asia/Qatar"}).format(new Date(b.start_at))} · {b.status}</div>{patientDetails ? <div className="mt-3 grid gap-1 rounded-xl bg-white/80 p-3 text-xs font-bold text-slate-600 sm:grid-cols-2"><span>المريض: <b className="text-slate-900">{patientDetails.patient_display_name}</b></span><span>الجنسية: <b className="text-slate-900">{patientDetails.patient_nationality}</b></span><span dir="ltr">ID: <b className="text-slate-900">{patientDetails.patient_national_id}</b></span><span dir="ltr">{patientDetails.patient_phone}</span></div> : <p className="mt-2 text-xs font-bold text-slate-400">بيانات المريض محجوبة وفق دورك.</p>}{attendanceReversed && b.status === "confirmed" && <div className="mt-2 text-xs font-bold text-amber-700">تم عكس حضور سابق؛ يمكن تسجيل الوصول من جديد عند حضور المريض.</div>}</div>
            <div className="flex flex-wrap gap-2">{canOperate ? <>
              ...
              {b.status === "checked_in" && !attendanceStateUnavailable && latestAttendance?.event_type === "checked_in" && <>
                ...
                {canReverseAttendance(b.branch_id) && <form action={reverseBookingCheckIn} className="flex gap-2"><input type="hidden" name="booking_id" value={b.id}/><Input name="reason" required minLength={3} placeholder="سبب عكس الحضور"/><Button className="bg-amber-600 px-3">عكس</Button></form>}
              </>}
              {mutableStatus && <ClinicBookingStatusForm bookingId={b.id} status={mutableStatus} startAt={b.start_at}/>} 
            </> : <Badge>عرض فقط</Badge>}</div>
          </div>;
