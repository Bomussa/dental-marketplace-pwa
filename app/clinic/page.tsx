...
import { applyClinic, createBranch, createClinicOperatorAccount, createOffer, createPractitioner, createSlot, publishOffer, publishSlot, requestPriceRevision, revokeClinicOperatorAccount, setDailyHours } from "./actions";
...
type Slot = { id: string; branch_id: string; variant_id: string; practitioner_id: string | null; start_at: string; end_at: string; status: string };
type Practitioner = { id: string; display_name: string; active: boolean; license_ref: string | null; clinic_id: string; gender: "female" | "male" | null };
...
function notificationDirectionsHref(payload: Record<string, unknown>) {
  const latitude = typeof payload.branch_latitude === "number" && Number.isFinite(payload.branch_latitude) ? payload.branch_latitude : null;
  const longitude = typeof payload.branch_longitude === "number" && Number.isFinite(payload.branch_longitude) ? payload.branch_longitude : null;
  if (latitude !== null && longitude !== null) return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${latitude},${longitude}`)}`;
  const address = [payload.branch_address, payload.branch_name, payload.branch_area, payload.clinic_name, "Qatar"].map(notificationText).filter((value): value is string => Boolean(value)).join(", ");
...
export default async function ClinicPage({ searchParams }: { searchParams: Promise<{ clinic?: string; activity_start?: string; activity_end?: string; activity_granularity?: string }> }) {
  ...
  const { data: membershipData, error: membershipError } = await withOperationalTimeout(supabase.from("clinic_memberships").select("clinic_id,branch_id,role,status").eq("status", "active"));
  const memberships = (membershipData ?? []) as Membership[];

  if (membershipError) return <main className="workspace-shell mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20"><Card className="border-amber-200 bg-amber-50/70 p-7 text-amber-950 sm:p-9"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-amber-700 ring-1 ring-amber-200"><ShieldCheckIcon size={24} /></span><h1 className="mt-5 text-2xl font-black">{copy.dashboardKicker}</h1><p role="alert" className="mt-3 text-sm font-bold leading-7">{copy.workspaceUnavailable}</p></Card></main>;
  ...
  const [{ data: offerData }, { data: slotData }, { data: variantData }, { data: bookingData }, { data: practitionerData }, { data: notificationData }] = await Promise.all([
    ...
    branchIds.length ? withOperationalTimeout(supabase.from("availability_slots").select("id,branch_id,variant_id,practitioner_id,start_at,end_at,status").in("branch_id", branchIds).order("start_at").limit(50)) : empty,
    ...
    withOperationalTimeout(supabase.from("practitioners").select("id,display_name,active,license_ref,clinic_id,gender").eq("clinic_id", selectedClinicId).limit(50)),
  ...
  const practitioners = (practitionerData ?? []) as Practitioner[];
  const practitionersById = new Map(practitioners.map((practitioner) => [practitioner.id, practitioner]));
  ...
  const latestAttendanceByBooking = attendanceStateUnavailable
    ? new Map<string, AttendanceEvent>()
    : latestClinicAttendanceByBooking((attendanceData ?? []) as unknown as AttendanceEvent[]);
  ...
  return <main className="workspace-shell mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12">
    ...
    <section className="mt-7 grid gap-6 lg:grid-cols-4">
      ...
      <Card className="p-5"><div className="flex items-center gap-2"><UserIcon size={18} className="text-[#0B5CAD]" /><h2 className="font-black">{copy.addPractitioner}</h2></div><form action={createPractitioner} className="mt-4 grid gap-3"><input type="hidden" name="clinic_id" value={selectedClinic?.id} /><Input name="display_name" required placeholder={copy.practitionerName} /><Input name="license_ref" placeholder={copy.licenseReference} /><Select name="gender" defaultValue=""><option value="">{copy.practitionerUndeclared}</option><option value="female">{copy.practitionerFemale}</option><option value="male">{copy.practitionerMale}</option></Select><Button disabled={!canManageClinic} title={!canManageClinic ? copy.restrictedAction : undefined}>{copy.saveAsPending}</Button></form><div className="mt-4 space-y-2 text-xs">{practitioners.map((practitioner) => <div key={practitioner.id} className="flex justify-between rounded-2xl bg-slate-50/80 p-3 ring-1 ring-slate-200/60"><span className="font-bold">{practitioner.display_name}<small className="ms-2 text-slate-500">{practitioner.gender === "female" ? copy.practitionerFemale : practitioner.gender === "male" ? copy.practitionerMale : copy.practitionerUndeclared}</small></span><Badge tone={practitioner.active ? "green" : "amber"}>{clinicStatus(locale, practitioner.active ? "active" : "pending")}</Badge></div>)}</div></Card>
