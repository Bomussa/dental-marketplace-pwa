...
type WaitlistRow = { id: string; status: string; created_at: string; notified_at: string | null; offer: { variant: { name_ar: string | null; name_en: string | null } | null } | null };
...
export async function AccountBookingNotifications({ locale, labels, notifications }: { locale: Locale; labels: NotificationLabels; notifications: NotificationRow[] }) {
  ...
  if (userId) {
    ...
    const { data } = await withOperationalTimeout(
      admin.from("booking_waitlist").select("id,status,created_at,notified_at,offer:branch_service_offers(variant:treatment_variants(name_ar,name_en))").eq("account_id", userId).in("status", ["active", "notified"]).order("created_at", { ascending: false }).limit(20),
    ).catch(() => ({ data: null }));
    waitlist = (data ?? []) as unknown as WaitlistRow[];
  }
  ...
  return (
    <>
      ...
      {waitlist.length > 0 && <section className="mt-8"><div className="mb-4"><p className="text-xs font-black uppercase tracking-[.18em] text-[#087d90]">{locale === "ar" ? "قائمة الانتظار" : "Waitlist"}</p><h2 className="mt-1 text-2xl font-black tracking-[-.025em] text-[#092b56]">{locale === "ar" ? "طلبات الانتظار الحالية" : "Your current waitlist requests"}</h2><p className="mt-1 text-sm font-medium text-slate-500">{locale === "ar" ? "يمكنك سحب أي طلب ما دام نشطًا. لن يتم حجز الموعد تلقائيًا." : "You can withdraw an active request. Appointments are never booked automatically."}</p></div><div className="space-y-3">{waitlist.map((entry) => { const variant = entry.offer?.variant; const treatmentName = locale === "ar" ? variant?.name_ar ?? variant?.name_en : variant?.name_en ?? variant?.name_ar; const status = entry.status === "notified" ? (locale === "ar" ? "تم إشعارك" : "Notified") : (locale === "ar" ? "نشط" : "Active"); return <Card key={entry.id} className="p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="font-black text-[#092b56]">{treatmentName ?? (locale === "ar" ? "طلب انتظار" : "Waitlist request")}</p><p className="mt-1 text-xs font-bold text-slate-500">{status} · {formatter.format(new Date(entry.created_at))}</p></div>{entry.status === "active" && <form action={withdrawBookingWaitlist}><input type="hidden" name="waitlist_id" value={entry.id} /><button type="submit" className="min-h-11 rounded-full border border-red-200 bg-white px-4 text-xs font-black text-red-700 transition hover:bg-red-50">{locale === "ar" ? "سحب الطلب" : "Withdraw request"}</button></form>}</div></Card>; })}</div></section>}
    </>
  );
}
